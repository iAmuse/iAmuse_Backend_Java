					...
					<div class="inner-content">
						<div class="col-row">
							<table class="table table-striped">
								...
								<tr>
								<input type="hidden" name="subId" value="${item.planCode}"/>
								<td>${item.subName}</td>
								<td>$ ${item.subPrice}</td>
								<td class="action_td">
											<span class="action_span"><a href="viewSubscription?id=${item.planCode}"><img src="<%=request.getContextPath()%>/resources/images/images/eye.png"></a></span>
											<span class="action_span"><a href="editSubscription?id=${item.planCode}"><img src="<%=request.getContextPath()%>/resources/images/images/edit-icon.png"></a></span>
											<span class="action_span"><a href="deleteSubscription?id=${item.planCode}"><img src="<%=request.getContextPath()%>/resources/images/images/delete-icon.png"></a></span>
											<c:if test="${item.status =='false'}">
											<span class="action_span"><a href="activeSubscription?id=${item.planCode}"><img src="<%=request.getContextPath()%>/resources/images/images/deactivate.png" width="20px"></a></span>
											</c:if>
											<c:if test="${item.status =='true'}">
											<span class="action_span"><a href="deactiveSubscription?id=${item.planCode}"><img src="<%=request.getContextPath()%>/resources/images/images/activate.png" width="20px"></a></span>
											</c:if>
								</td>
