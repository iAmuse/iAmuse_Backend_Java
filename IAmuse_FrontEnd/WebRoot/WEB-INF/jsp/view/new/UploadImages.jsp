	...
	<img src="http://www.willmaster.com/images/preload.gif">
			...
			<div class="right-pannel">
				 <c:if test="${not empty successMessage}">
					...
					<p class="subtext">This Event Zoom Profile (<b><c:if test="${eventZoomScale==1.00}">Zoomed In</c:if><c:if test="${eventZoomScale==0.50 || eventZoomScale==0.5}">Zoomed Out</c:if><c:if test="${eventZoomScale==0.75}">Medium Zoom</c:if></b>)</p>
					<c:if test="${boothAdminLogin.planCode!='1'}"><a href="javascript:void(0)" class="btn btn-green pull-right"  onclick="open1();">Add Background</a></c:if>
  	...
  	<center style="margin: 10px 0px 30px;"><b>current zoom scale &nbsp; (<c:if test="${fovbyuser.zoomScale==1.00}">Zoomed In</c:if><c:if test="${fovbyuser.zoomScale==0.50 || fovbyuser.zoomScale==0.5}">Zoomed Out</c:if><c:if test="${fovbyuser.zoomScale==0.75}">Medium Zoom</c:if>)</b><br/></center>
...
<button onclick="closeThankYouScreen();" style="float:right;margin-right:10px;" class="btn btn-green">Done</button>
					    ...
					    <div class="modal-dialog">
					        <div class="modal-content" style="width:120% !important;">
					            <div class="modal-header">
					                ...
					                <p class="subtext">Current Camera Screen size: <c:forEach items="${deviceRegistration}" var="cr"><c:if test="${cr.deviceType=='Camera device'}">${cr.deteactedResolution}</c:if> </c:forEach> (Final email attachment dimensions dictated by Camera device's screen dimensions, regardless of Camera Megapixels.)</p>
					            </div>
											...
											<input type="hidden" value="${eid}" name="eId"/>
												<span class="upload_file" >
													<div class="file_container" data-step="3" data-intro="Upload one or multiple events by keeping the control key pressed" data-toggle="tooltip" title="Upload one or multiple events by keeping the control key pressed">
														<input type="file" name="files" id="images2" multiple accept=".jpg,.png,.gif,.bmp" onchange="return updateList()" onclick="return validateZoom();" />
					...
					<h1 class="custom-heading">Backgrounds for the Event</h1>
						<div class="col-row">
						<form:form action="setUpBackgroundImage" modelAttribute="AdminPictureVO" onclick="return validateId();">
								...
								<c:if test="${notConfiguredImage!=0}"><p class="subtext" style="margin-bottom: 10px;color: red;">You have not configured <b>${notConfiguredImage}</b> out of <b>${adminPictureVOs2.size()}</b> backgrounds</p></c:if>
									<!-- <div class="jcarousel" style="padding-left:5px !important" id="slideshowContainer">
										...
										<c:forEach items="${adminPictureVOs2}" var="picBackground" varStatus="loop" >
											<c:if test="${picBackground.updatedDate !=null}">
										            <div class="col-sm-6 col-md-3 ">
										            	<div class="grid_view_img radio_cross"><img src="${picBackground.picName}"><input type="radio" name="picId" class="configImg" value="${picBackground.picId}" onclick="setPosition(${loop.index+1})"><c:if test="${boothAdminLogin.planCode!='1'}"><span id="crossId" class="configImg"  style="float: right;cursor: pointer;position: absolute;right: 10px;" onclick="deletEventPicture('${picBackground.picId}','${eid}')"  >&#10005;</span></c:if></div>
										    ...
										    <c:if test="${picBackground.updatedDate ==null}">
										        <div class="col-sm-6 col-md-3 ">
										            <div class="grid_view_img radio_cross" style="border: 2px solid red;">
										            	<img src="${picBackground.picName}"><input type="radio" name="picId" class="configImg" value="${picBackground.picId}" onclick="setPosition(${loop.index+1})"><c:if test="${boothAdminLogin.planCode!='1'}"><span id="crossId" class="configImg"  style="float: right;cursor: pointer;position: absolute;right: 10px;" onclick="deletEventPicture('${picBackground.picId}','${eid}')"  >&#10005;</span></c:if>
										            </div>
										            <p class="subtext event_thumbnail_text" style="">${picBackground.picTitle}</p>
										        </div>
										    </c:if>
					 ...
					 <form:form action="updateWaterMarkLookAtTouchThankYouCameraScreen" modelAttribute="AdminPictureVO" onsubmit="return validateUpdateForm();" enctype="multipart/form-data" method="post">
						...
						<div class="upload-imgs">
							<div class="form-row upload-screens">
										...
										<div>
											<span>	End of Session</span><c:if test="${boothAdminLogin.planCode!='1'}"><input type="file" name="thankyoufiles" id="thankyoufilesUploadFile"  accept=".jpg,.png,.gif,.bmp" onchange="javascript:updateThankYouList()" /></c:if>
											<div class="clearfix"></div>
										</div>
										...
										<div>
											<span>Photos Complete</span><c:if test="${boothAdminLogin.planCode!='1'}"><input type="file" name="lookAtTouchScreen" id="lookAtTouchScreenUploadFile"  accept=".jpg,.png,.gif,.bmp" onchange="javascript:updatelookAtTouchScreenList()" /></c:if>
											<div class="clearfix"></div>
										</div>
										
										<div>
								         	<span>Screensaver</span><c:if test="${boothAdminLogin.planCode!='1'}"><input type="file" name="cameraTVScreenSaver" id="cameraTVScreenSaverUploadFile"  accept=".jpg,.png,.gif,.bmp" onchange="javascript:updateCameraTvScreenSaverList()" /></c:if>
								        	<div class="clearfix"></div>
								         </div>
								         
								         	<div>
										<span>Background Overlay</span><c:if test="${boothAdminLogin.planCode!='1'}"><input type="file" name="waterMarkImage" id="waterMarkImageUploadFile"  accept=".jpg,.png,.gif,.bmp" onchange="javascript:updateWaterMarkImageList()" /></c:if>
										<div class="clearfix"></div>
										</div>
										<div class="clearfix"></div>
									</div>
