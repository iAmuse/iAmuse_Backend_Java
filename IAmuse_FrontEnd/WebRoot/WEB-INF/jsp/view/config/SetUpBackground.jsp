...
<c:if test="${boothAdminLogin2.planCode ==1}">
<script>
$( document ).ready(function() {
...
<div class="right-pannel">
		<h1 class="heading pull-left" style="margin-bottom:15px;">Setup Custom Background</h1>
<form:form action="saveCoordinatesOfImgConfig" modelAttribute="AdminPictureVO" enctype="multipart/form-data">
					...
					<h2 class="heading" style="margin: -15px 0px 15px 0px;"> Background ${position} of ${adminPictureVOs2.size()} </h2>
						<div class="col-row">
							<div class="background-priview" id="target" data-toggle="tooltip" title="You can drag, resize the window to the location where you want the camera image to appear. You can also use the zoom profile. You can also upload Masking image and Watermark image. Click on Done after you have made all configurations">
								<div class="background-image">
									<div class="img-container cropper-bg"  data-step="2" data-intro="You can drag, resize the window to the location where you want the camera image to appear. You can also use the zoom profile. You can also upload Masking image and Watermark image. Click on Done after you have made all configurations">
							...
							<input type="hidden" name="cropImgX" id="x" value="${adminPictureVO1.cropImgX}"/>
						 	...
						 	<input type="hidden" name="cropImgHeight" id="h2" value="${adminPictureVO1.cropImgHeight}"/>	
								...
								<div class="img-position-controls">
									...
									<div class="control">
										<span>Position</span><img src="<%=request.getContextPath()%>/resources/images/images/position.png">
									</div> 
									<div class="control">
							...
							<c:if test="${boothAdminLogin2.planCode ==1}"><center><p class="subtext">This Background Is Default Background You Can't Configure According To Your Choice .</p><div class="clearfix"></div></center></c:if>
...
<a href="${adminPictureVO1.imageWaterMark}" class="Watermark">Watermark Image</a><span class="Watermark" style="cursor: pointer;" onclick="updateWaterMarkStatus();">&nbsp;&nbsp;&#10005;</span>
								...
								<input type="submit" class="btn btn-green" style="margin-top:25px;width:auto;display:none; " id="uploadNow" name="finish" value="Upload Now" data-toggle="tooltip" title="You can select the next image to configure else You are now ready to run your booth"/> 
								</div></center>
							</div>
							<div class="clearfix"></div>
						</div>
						...
						<input type="hidden" value="${eid}" name="eId"/>
								...
								<c:set var="first" value="${first}"/>
      							...
      							<c:set var="current" value="${current}"/>
									<%-- <c:if test="${first != current}"><a href="setUpBackgroundImageConfig?picId=${previous}&eId=${eid}&position=${position-1}" class="btn btn-green" style="padding:10px;background-color:#05a42e ;color:#fff;margin-right: 10px;margin-top:25px">Previous</a></c:if> --%>
									...
									<input type="submit" class="btn btn-green pull-right" style="margin-top:25px;width:auto;" value="Save & Exit" data-toggle="tooltip" name="finish" title="You can select the next image to configure else You are now ready to run your booth"/>
					</c:if>
					<div class="clearfix"></div>
					</div>
					</div>
