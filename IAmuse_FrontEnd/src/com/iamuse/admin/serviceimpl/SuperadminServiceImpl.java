...
public class SuperadminServiceImpl implements SuperadminService {
		...
		public List<BoothAdminLoginResponseVO> getBoothAdminLoginList() {
			...
			for (BoothAdminLogin boothAdminLogin : login) {
				...
				response.setContactNumber(DateUtil.checkNull(boothAdminLogin.getContactNumber()));
				response.setCreatedDate(DateUtil.timeStampConvertIntoStringDateFormatYYYYMMDD(boothAdminLogin.getCreatedDate()));
				response.setEmailId(DateUtil.checkNull(boothAdminLogin.getEmailId()));
				response.setStatus(boothAdminLogin.getStatus());
				response.setPlanCode(boothAdminLogin.getPlanCode());
				if(boothAdminLogin.getSubUpdatedDate()!=null){
				response.setSubUpdatedDate(DateUtil.timeStampConvertIntoStringDateFormatYYYYMMDD(boothAdminLogin.getSubUpdatedDate()));
				}
				if(boothAdminLogin.getUpdatedDate()!=null){
				response.setUpdatedDate(DateUtil.timeStampConvertIntoStringDateFormatYYYYMMDD(boothAdminLogin.getUpdatedDate()));
				}
		...
		public EventVO getEventValueByEventId(Integer eventId) {
			 ...
			 vo.setEventStart(DateUtil.timeStampConvertIntoStringDateFormatYYYYMMDD(event.getEventStart()));
			 }
		...
		public List<BoothAdminLoginResponseVO> getBoothAdminLoginListSuperAdmin() {
			...
			for (BoothAdminLogin boothAdminLogin : login) {
				...
				response.setContactNumber(DateUtil.checkNull(boothAdminLogin.getContactNumber()));
				response.setCreatedDate(DateUtil.timeStampConvertIntoStringDateFormatYYYYMMDD(new Date()));
				response.setEmailId(DateUtil.checkNull(boothAdminLogin.getEmailId()));
				response.setIsDeleted(boothAdminLogin.getIsDeleted());
				response.setLocation(DateUtil.checkNull(boothAdminLogin.getLocation()));
				response.setStatus(boothAdminLogin.getStatus());
				response.setPlanCode(boothAdminLogin.getPlanCode());
				if(boothAdminLogin.getSubEndDate()!=null){
				    ...
				    Date startDate;
					try {
						startDate = formatter.parse(todayDate);
						if(startDate.compareTo(boothAdminLogin.getSubEndDate())>0){
							listUserIds.add(boothAdminLogin.getUserId());
						}
					} catch (ParseException e) {
						e.printStackTrace();
					}
					response.setSubEndDateFormat(DateUtil.timeStampConvertIntoStringDateFormatYYYYMMDD(boothAdminLogin.getSubEndDate()));
				}
			...
			superadminDao.updateExpiredUsers(listUserIds);
			return responseList;
		}
