...
public class NVPAPICaller extends APICallerBase
{
    ...
    public final synchronized void setupConnection(APIProfile _profile) throws PayPalException
    {
        ...
        String endpointUrl = null;
            ...
            endpointUrl = "https://api-3t." + _profile.getEnvironment() + ".paypal.com/nvp";
       
        
        if ((endpointUrl == null) & (Util.isEmpty(endpointUrl)))
        {
            throw new TransactionException(MessageFormat.format(MessageResources.getMessage("ENDPOINT_NOT_FOUND"), new Object[]
                    {
        ...
        try
        {
            System.out.println("**DEBUG : end point URL : " + endpointUrl);
            url = new URL(endpointUrl);
        }
        catch (Exception e)
        {
            throw new TransactionException(MessageFormat.format(MessageResources.getMessage("ENDPOINT_INVALID"), new Object[]
                    {
                        endpointUrl
                    }));
        }
        ...
        if (log.isDebugEnabled())
        {
            log.debug(MessageFormat.format(MessageResources.getMessage("CONNECTION_OPEN"),
                                           new Object[]
                    {
                        service.getLocalPart(), endpointUrl
                    }));
        }
